...
def forecast_core(station_id, horizon, df_all, model, meta):
    try:
        sid = int(station_id)
    except:
        sid = station_id
    hist = df_all[df_all["start_station_id"]
                  == sid].sort_values("trip_date").copy()
    if hist.empty:
        raise ValueError("station not found")
    rides_hist = hist["rides"].astype(float).tolist()
    while len(rides_hist) < 28:
        rides_hist.insert(0, 0.0)
    sid_to_idx = make_station_index(df_all)
    station_idx = sid_to_idx.get(sid, 0)
    last_date = df_all["trip_date"].max()
    preds, future_dates = [], []
...
def forecast_endpoint(req: ForecastRequest):
    try:
        ...
        fc = forecast_core(req.station, req.horizon, df_all, model, meta)
        fc["trip_date"] = fc["trip_date"].dt.strftime("%Y-%m-%d")
        data = [ForecastRow(trip_date=r["trip_date"], pred_rides=float(
            r["pred_rides"])) for _, r in fc.iterrows()]
        return {"station": req.station, "horizon": req.horizon, "data": data}
